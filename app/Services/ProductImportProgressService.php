...
class ProductImportProgressService
{
    ...
    protected function shouldFailAsZombie(string $uploadId, array $progress, ?int $secondsSinceUpdate): bool
    {
        ...
        if (! app(ProductImportLockService::class)->hasActiveWork($uploadId)) {
            return false;
        }
    ...
    public function complete(string $uploadId, array $process): void
    {
        ...
        if (isset($process['run_id'])) {
            $run = ProductImportRun::query()->find((int) $process['run_id']);
            if ($run !== null) {
                $redirect = app(ProductImportRunService::class)->redirectUrlForRun($run);
            }
        }
    ...
    {
        return 'product_import_progress:'.$uploadId;
    }
}
