...
class ShippingWeightRateChunkUploadService
{
    public const MAX_CHUNK_BYTES = 6291456;

    public const MAX_TOTAL_BYTES = 52428800;

    public function __construct(protected ShippingWeightRateImportJobService $importJob) {}
    ...
    {
        $dir = $this->uploadDirectory($uploadId);
        File::ensureDirectoryExists(storage_path('app/imports/shipping-merged'));

        $mergedPath = storage_path('app/imports/shipping-merged/'.$uploadId.'.csv');
    ...
    {
        return storage_path('app/imports/shipping-chunks/'.$uploadId);
    }
