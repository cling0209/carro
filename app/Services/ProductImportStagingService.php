...
class ProductImportStagingService
{
    ...
    public function storeFromMergedPath(
        ...
        if ($preparer->isSpreadsheetPath($mergedPath, $originalName)) {
            $metadata = app(ProductSpreadsheetReader::class)->readMetadata($stagingPath);
        ...
        ProductImportStaging::query()->where('upload_id', $uploadId)->delete();

        ProductImportStaging::query()->create([
            'upload_id' => $uploadId,
            'user_id' => $userId,
            'username' => $username,
        ...
        return [
            ...
            'columns' => $columns,
            'total_rows' => $totalRows,
            'suggested_mapping' => ProductImportColumnMapping::suggest($columns),
        ];
    }
    ...
    {
        $existing = ProductImportStaging::query()->where('upload_id', $uploadId)->first();
        ...
        if ($existing !== null) {
            ...
            return [
                ...
                'suggested_mapping' => ProductImportColumnMapping::suggest($existing->columns ?? []),
            ];
        }
    ...
    public function storeFromCsvContent(
        ...
        $importService = app(ProductCatalogImportService::class);
    ...
    {
        $progress = app(ProductImportJobService::class)->continuePrepareCustom($uploadId, $userId, $mapping);
    ...
    public function findStagingRecord(string $uploadId): ProductImportStaging
    {
    ...
    public function preview(string $uploadId, int $userId, array $mapping, int $limit = 10): array
    {
        ...
        ProductImportColumnMapping::validate($mapping);
    ...
    {
        $staging = ProductImportStaging::query()->find($uploadId);
    ...
    protected function findStaging(string $uploadId): ProductImportStaging
    {
        $this->assertValidUploadId($uploadId);
        $this->purgeExpired();
    ...
    {
        $expired = ProductImportStaging::query()
