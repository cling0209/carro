...
class ProductImportJobService
{
    ...
    protected function finalizeStreamJobFromCsvPath(
        ...
        $preparer = app(ProductImportStreamPreparer::class);
    ...
    public function continuePrepareTemplate(string $uploadId, int $userId): array
    {
        ...
        $pendingService = app(ProductImportPendingService::class);
        $preparer = app(ProductImportStreamPreparer::class);
        $jobDir = $this->jobDirectory($uploadId);
        File::ensureDirectoryExists($jobDir);
        ...
        try {
            if (! File::exists($statePath)) {
                $metadata = app(ProductSpreadsheetReader::class)->readMetadata($sourcePath);
    ...
    public function queueBackgroundImport(
        ...
        $progress = app(ProductImportProgressService::class);
        $lock = app(ProductImportLockService::class);
    ...
    public function runBackgroundImport(
        ...
        try {
            ...
            if (! File::exists($this->jobDirectory($uploadId).'/job.json')) {
                $progress->setPhase(
                    $uploadId,
                    ProductImportProgressService::PHASE_PREPARE,
            ...
            $progress->setPhase(
                $uploadId,
                ProductImportProgressService::PHASE_PROCESS,
    ...
    public function processNextBatch(string $uploadId, int $userId): array
    {
        ...
        $batchResult = app(ProductCatalogImportService::class)->importFromUploadedFile(
            $uploaded,
            $job['username'] ?? null,
            $columnMapping,
        );
    ...
    public function processNextBatchWithRun(string $uploadId, int $userId): array
    {
        ...
        $runService = app(ProductImportRunService::class);
        ...
        try {
            ...
            if ($progress['finished'] && $runId !== null) {
                $run = $runService->completeRun(
                    ProductImportRun::query()->findOrFail($runId),
                    $progress['result'],
                );
    ...
    protected function mergeResults(array $current, array $batch): array
    {
        return [
            ...
            'updated' => $current['updated'] + $batch['updated'] + (int) ($batch['reactivated'] ?? 0),
    ...
    public function continuePrepareCustom(
        ...
        @set_time_limit(180);
        @ini_set('memory_limit', '512M');
        $stagingService = app(ProductImportStagingService::class);
        ...
        ProductImportColumnMapping::validate($columnMapping);
    ...
    {
        return app(ProductCatalogImportService::class)->emptyResult();
    }
    ...
    protected function processNextStreamChunk(string $uploadId, array $job): array
    {
        ...
        $streamResult = app(ProductCatalogImportService::class)->importFromCsvStreamPath(
            $sourcePath,
    ...
    protected function runSpreadsheetPrepareChunk(
        ...
        array $state,
        string $statePath,
        ProductImportPendingService|ProductImportStagingService $cleanupService,
    ): array {
        $jobDir = $this->jobDirectory($uploadId);
        ...
        try {
            ...
            if ($cleanupService instanceof ProductImportPendingService) {
    ...
    protected function beginExcelDirectJob(
        ...
        $reader = app(ProductSpreadsheetReader::class);
    ...
    protected function processNextExcelDirectChunk(string $uploadId, array $job): array
    {
        ...
        $chunkResult = app(ProductCatalogImportService::class)->importRows(
            $rows,
            $job['username'] ?? null,
            $columnMapping,
        );
    ...
    protected function assertReadyForBackgroundImport(string $uploadId, int $userId, string $mode): void
    {
        ...
        if ($mode === 'custom' && ProductImportStaging::query()->where('upload_id', $uploadId)->exists()) {
            return;
        }
    ...
    protected function canUseFullOpenSpoutPrepare(string $uploadId, int $userId, string $mode): bool
    {
        ...
        return app(ProductOpenSpoutReader::class)->supportsPath(
            $source['path'],
    ...
    protected function resolvePrepareSource(string $uploadId, int $userId, string $mode): ?array
    {
        ...
        if ($mode === 'custom') {
            $staging = app(ProductImportStagingService::class)->findStagingRecord($uploadId);
    ...
    protected function prepareSpreadsheetInWorker(
        ...
        $openSpout = app(ProductOpenSpoutReader::class);
        ...
        if (app(ProductImportPendingService::class)->has($uploadId)) {
            app(ProductImportPendingService::class)->consume($uploadId);
        } elseif ($mode === 'custom') {
            app(ProductImportStagingService::class)->cleanup($uploadId);
        }
