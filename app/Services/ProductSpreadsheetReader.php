...
use App\Support\ProductSpreadsheetRowReadFilter;
...
class ProductSpreadsheetReader
{
    ...
    public function readMetadata(string $path): array
    {
        ...
        $reader->setReadFilter(new ProductSpreadsheetRowReadFilter(1, 1));
    ...
    public function readDataRows(string $path, int $startRow, int $endRow, array $headers, int $columnCount): array
    {
        ...
        $reader = $this->createReader($path);
        $reader->setReadFilter(new ProductSpreadsheetRowReadFilter($startRow, $endRow));
        $spreadsheet = $reader->load($path);
        $sheet = $spreadsheet->getActiveSheet();
        $rows = [];
